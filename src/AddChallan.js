...
function AddChallan() {
  ...
  const add = async () => {
    let t1=await removeDup();

    let policeOfficer = localStorage.getItem('id');
    ...
    web3.eth.personal.unlockAccount(policeOfficer, pswd, 50000).then(
      () => {
        let data=new Date();
        contract.methods.addChallan(License, Name, CNIC, CarType, Plate, [0, 1], data.toString() )
          .send({
            from: policeOfficer,
            gasPrice: 90000,
            gas: 890000
